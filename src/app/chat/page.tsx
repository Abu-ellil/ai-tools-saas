...
export default function ChatPage() {
  ...
  // حالة النموذج المحدد - استخدام GPT-3.5 Turbo كنموذج افتراضي
  const [selectedModel, setSelectedModel] = useState<Model>(
    // البحث عن نموذج GPT-3.5 Turbo
    availableModels.find((model) => model.id === "gpt-3.5-turbo") ||
      availableModels[0]
  );
      ...
      const errorMessage = {
        ...
        content: `عذراً، حدث خطأ أثناء معالجة طلبك. ${
          error instanceof Error && error.message.includes("429")
            ? "لقد تجاوزت الحصة المخصصة لك في OpenAI API. يرجى التحقق من مفتاح API الخاص بك."
  ...
  return (
    <div className="container py-10">
      <div className="max-w-4xl mx-auto">
        ...
        {/* عرض تنبيه عندما يكون مفتاح API غير صالح */}
        {(messages.some(
          (msg) =>
            msg.role === "assistant" &&
            (msg.content.includes(
              "لقد تجاوزت الحصة المخصصة لك في OpenAI API"
            ) ||
              msg.content.includes("مفتاح OpenAI API غير صالح") ||
              msg.content.includes("429") ||
              msg.content.includes("هذا رد وهمي لأغراض التطوير"))
        ) ||
          process.env.OPENAI_API_KEY ===
            "sk-demo-key-for-development-only") && (
          <div className="mb-6">
            <ApiKeyAlert service="openai" />
          </div>
        )}
